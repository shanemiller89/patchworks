...
export async function renderMainMenu(options) {
    ...
    const showExcluded = resolveBooleanOption(options.showExcluded, config?.showExcluded, false);
    ...
    const renderMenu = () => {
        console.clear();
        console.log(boxen(chalk.blue.bold(MAIN_TITLE), {
            title: chalk.blue('Stitching Your Changelog Chaos into Seamless Updates.'),
...
${chalk.blue(DOUBLE_LINE)}
  ${chalk.white('★')} ${chalk.gray.bold('Update Level:')}               ${!level ? chalk.redBright('Not Set') : chalk.greenBright(level)}
  ${chalk.white('♯')} ${chalk.gray.bold('Max Updates:')}                ${chalk.greenBright(limit || 'Infinite')}
  ${chalk.white('◈')} ${chalk.gray.bold('Dependency Scope:')}           ${chalk.greenBright(levelScope)}
${chalk.blue(SINGLE_LINE)}
  ${chalk.white('✿')} ${chalk.gray.bold('Summary Mode:')}               ${summary ? chalk.green('Enabled') : chalk.red('Disabled')}
  ${chalk.white('☒')} ${chalk.gray.bold('Show Excluded:')}              ${showExcluded ? chalk.green('Enabled') : chalk.red('Disabled')}
  ${chalk.white('♨')} ${chalk.gray.bold('Install Dependencies:')}            ${install ? chalk.green('Enabled') : chalk.red('Disabled')}
${chalk.blue(DOUBLE_LINE)}
  `;
        ...
        const updateTable = table([
            ...
            [
                selectedIndex === 1
                    ? chalk.bgCyanBright.bold.blackBright('↻ [2] Run Patchworks Main All')
        ...
        const otherTable = table([
            ...
            [
                selectedIndex === 5
                    ? chalk.bgCyanBright.bold.blackBright('☉ [6] Help and Documentation')
        ...
        console.log(chalk.blue(`Use ${chalk.bold.blueBright('↑ ↓')} keys to navigate, press ${chalk.bold.blueBright('Enter')} to select.`));
    };
