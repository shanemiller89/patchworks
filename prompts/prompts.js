        ...
        const { level } = await inquirer.prompt([
            {
                ...
                choices: [
                    {
                        name: `${styles.patch('Patch (x.x.1)')} - Bug fixes and minor updates`,
                        value: 'patch',
                    },
                    {
                        name: `${styles.minor('Minor (x.1.x)')} - New features (backwards compatible)`,
                        value: 'minor',
                    },
                    {
                        name: `${styles.major('Major (1.x.x)')} - High potential for Breaking changes`,
                        value: 'major',
                    },
                ],
            },
    ...
    try {
        ...
        if (useCustomDir) {
            ...
            const createReportsFolder = await task
                .prompt(ListrInquirerPromptAdapter)
                .run(customTogglePrompt, {
                message: 'Do you want to create a "patchworks-reports" folder in the selected directory?',
                default: true,
                active: 'Yes',
                inactive: 'No',
            });
        ...
        logger.error(`An error occurred during the directory prompt: ${error.message}`);
    ...
    try {
        ...
        const choices = packages.map((pkg) => {
            ...
            return {
                ...
                message: `${pkg.packageName} (Current: ${pkg.metadata.current} => Latest: ${pkg.metadata.latest} - ${styledUpdateType} (Score of ${styles.generic(updatingDifficulty.toString())}) - Potential Breaking Changes: ${hasBreakingChange}`,
        ...
        const selectedPackageNames = await task
            ...
            choices,
            validate: (value) => value.length ? true : 'You must select at least one package.',
        });
        const selectedPackages = packages.filter((pkg) => selectedPackageNames.includes(pkg.packageName));
