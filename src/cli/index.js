...
function createLevelArg(options = { menu: false }) {
    ...
    if (menu) {
        return new Argument('[level]', 'Specify the update level (patch, minor, major)').argParser((value) => {
...
const sharedOptions = [
    new Option('-n, --limit <number>', 'Limit the number of updates processed').argParser((value) => {
    ...
    new Option('--level-scope <scope>', 'Control semantic version filtering (strict or cascade)').argParser((value) => {
    ...
    new Option('--exclude-repoless', 'Exclude packages without repositories'),
    new Option('-d, --debug', 'Show verbose debug consoles'),
    new Option('--show-excluded', 'Show excluded packages in the console output'),
...
export default function () {
    ...
    program
        .name('patchworks')
        .description('Patchworks CLI: A utility for managing dependency updates efficiently.')
    ...
    program
        ...
        const finalOptions = {
            ...
            excludeRepoless: resolveBooleanOption(options.excludeRepoless, config?.excludeRepoless, false),
            debug: resolveBooleanOption(options.debug, config?.debug, false),
            showExcluded: resolveBooleanOption(options.showExcluded, config?.showExcluded, false),
        };
