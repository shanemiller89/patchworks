 ...
 * @returns A formatted changelog or null if no data is found.
 */
export async function fetchCommits({
  ...
  try {
    ...
    if (!tags || tags.length === 0) {
      logger.warn(`No tags found for ${owner}/${repo}. Skipping commit fetching.`);
      return null;
    }
...
export function formatChangelog(
  conventionalCommits: ParsedCommit[],
  nonConventionalCommits: Commit[]
): string | null {
...
function generateConventionalChangelog(commits: ParsedCommit[]): string | null {
